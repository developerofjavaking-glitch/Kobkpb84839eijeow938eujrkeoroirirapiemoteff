import requests, os, sys, json, binascii, time, urllib3, base64, re, threading, random
from protobuf_decoder.protobuf_decoder import Parser
from xC4 import (EnC_AEs_sync as EnC_AEs, EnC_Uid_sync as EnC_Uid,
                 DeCode_PackEt_sync as DeCode_PackEt, xMsGFixinG)
...
def equie_emote(JWT, url):
    ...
    try:
        requests.post(url, headers=headers, data=data, timeout=10, verify=False)
    except Exception as e:
...
def Likes(id):
    try:
        ...
        name, lvl, lb, la, lg = (get(r).group(1) if get(r) else None for r in
            [r"PLayer NamE\s*:\s*(.+)", r"PLayer SerVer\s*:\s*(.+)",
...
def Requests_SPam(id):
    try:
        Api = requests.get(
            ...
            timeout=15)
        return Api.status_code in [200, 201] and '[SuccessFuLy] -> SenDinG Spam ReQuesTs !' in Api.text
    except:
        return False
...
def GeT_Name(uid, Token):
    try:
        ...
        headers = {
            ...
            'Authorization': f'Bearer {GeTToK()}', 'Content-Length': '16',
            'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 7.1.2; ASUS_Z01QD Build/QKQ1.190825.002)',
            'Host': 'clientbp.ggblueshark.com', 'Connection': 'Keep-Alive', 'Accept-Encoding': 'gzip'}
...
def GeT_PLayer_InFo(uid, Token):
    try:
        ...
        url = "https://clientbp.common.ggbluefox.com/GetPlayerPersonalShow"
        headers = {
            'X-Unity-Version': '2018.4.11f1', 'ReleaseVersion': 'OB50',
            'Content-Type': 'application/x-www-form-urlencoded', 'X-GA': 'v1 1',
            'Authorization': f'Bearer {GeTToK()}', 'Content-Length': '16',
            'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 7.1.2; ASUS_Z01QD Build/QKQ1.190825.002)',
            'Host': 'clientbp.ggblueshark.com', 'Connection': 'Keep-Alive', 'Accept-Encoding': 'gzip'}
...
def DeLet_Uid(id, Tok):
    ...
    headers = {
        'X-Unity-Version': '2018.4.11f1', 'ReleaseVersion': 'OB50',
        'Content-Type': 'application/x-www-form-urlencoded', 'X-GA': 'v1 1',
        'Authorization': f'Bearer {Tok}', 'Content-Length': '16',
        'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 7.1.2; ASUS_Z01QD Build/QKQ1.190825.002)',
        'Host': 'clientbp.ggblueshark.com', 'Connection': 'Keep-Alive', 'Accept-Encoding': 'gzip'}
