...
# ── sync helpers used in xHeaders (non-async context) ──────────────────────
def EnC_AEs_sync(HeX):
    cipher = AES.new(Key, AES.MODE_CBC, Iv)
    return cipher.encrypt(pad(bytes.fromhex(HeX), AES.block_size)).hex()

def DEc_AEs_sync(HeX):
    cipher = AES.new(Key, AES.MODE_CBC, Iv)
...
async def EnC_AEs(HeX):
    return EnC_AEs_sync(HeX)

async def DEc_AEs(HeX):
    return DEc_AEs_sync(HeX)
...
async def Ua():
    ...
    models = ['SM-A125F','SM-A225F','SM-A325M','SM-A515F','SM-A725F','SM-M215F',
              'SM-M325FV','Redmi 9A','Redmi 9C','POCO M3','POCO M4 Pro','RMX2185',
              'RMX3085','moto g(9) play','CPH2239','V2027','OnePlus Nord','ASUS_Z01QD']
...
async def ArA_CoLor():
    Tp = ["32CD32","00BFFF","00FA9A","90EE90","FF4500","FF6347","FF69B4","FF8C00",
...
async def xBunnEr():
    bN = [902000306,902000305,902000003,902000016,902000017,902000019,902031010,
          902043025,902043024,902000020,902000021,902000023,902000070,902000087,
          902000108,902000011,902049020,902049018,902049017,902049016,902049015,
          902049003,902033016,902033017,902033018,902048018,902000306,902000305,902000079]
    return random.choice(bN)

async def xSEndMsg(Msg, Tp, Tp2, id, K, V):
    feilds = {1: id, 2: Tp2, 3: Tp, 4: Msg, 5: 1735129800, 7: 2,
              9: {1: "xBesTo - C4", 2: int(await xBunnEr()), 3: 901048018, 4: 330,
                  5: 909034009, 8: "xBesTo - C4", 10: 1, 11: 1, 13: {1: 2},
                  14: {1: 12484827014, 2: 8,
                       3: "\u0010\u0015\b\n\u000b\u0013\f\u000f\u0011\u0004\u0007\u0002\u0003\r\u000e\u0012\u0001\u0005\u0006"},
                  12: 0},
              10: "en", 13: {3: 1}}
...
async def xSEndMsgsQ(Msg, id, K, V):
    fields = {1: id, 2: id, 4: Msg, 5: 1756580149, 7: 2, 8: 904990072,
              9: {1: "xBe4!sTo - C4", 2: await xBunnEr(), 4: 330, 5: 827001005,
...
async def SPam_Room(Uid, Rm, Nm, K, V):
    fields = {1: 78, 2: {1: int(Rm), 2: f"[{await ArA_CoLor()}]{Nm}", 3: {2: 1, 3: 1},
                         4: 330, 5: 1, 6: 201, 10: await xBunnEr(), 11: int(Uid), 12: 1}}
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), '0e15', K, V)

async def GenJoinSquadsPacket(code, K, V):
    fields = {1: 4, 2: {4: bytes.fromhex("01090a0b121920"), 5: str(code), 6: 6, 8: 1,
...
async def Msg_Sq(msg, owner, bot, K, V):
    fields = {1: 1, 2: {1: bot, 2: owner, 4: msg, 5: 1757799182, 7: 2,
                        9: {1: "Fun1w5a2", 2: await xBunnEr(), 3: 909000024,
                            4: 330, 5: 909000024, 7: 2, 10: 1, 11: 1, 12: 0,
                            13: {1: 2},
                            14: {1: bot, 2: 8,
                                 3: "\u0010\u0015\b\n\u000b\u0013\f\u000f\u0011\u0004\u0007\u0002\u0003\r\u000e\u0012\u0001\u0005\u0006"}},
                        10: "ar", 13: {3: 1}, 14: ""}}
...
async def ghost_pakcet(player_id, secret_code, K, V):
    fields = {1: 61, 2: {1: int(player_id),
                         2: {1: int(player_id), 2: int(time.time()), 3: "MR3SKR",
                             5: 12, 6: 9999999, 7: 1, 8: {2: 1, 3: 1}, 9: 3},
                         3: secret_code}}
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), '0515', K, V)
